...
// 	lua_pushstring(L, temp);
	...
	std::string temp = base::Int64ToString(*reinterpret_cast<const int64*>(lua_topointer(L, 1)));
...
static int eq_s64(lua_State *L)
{
	...
	int64 a = *reinterpret_cast<const int64*>(lua_touserdata(L, 1));
	int64 b = *reinterpret_cast<const int64*>(lua_touserdata(L, 2));
...
static int tostring_u64(lua_State *L)
{
	...
	std::string temp = base::Uint64ToString(*reinterpret_cast<const uint64*>(lua_topointer(L, 1)));
	lua_pushstring(L, temp.c_str());
	return 1;
}
...
static int eq_u64(lua_State *L)
{
	...
	uint64 a = *reinterpret_cast<const uint64*>(lua_touserdata(L, 1));
	uint64 b = *reinterpret_cast<const uint64*>(lua_touserdata(L, 2));
	lua_pushboolean(L, (a == b));
	return 1;
}
